...
const NewProduct: NextPageWithLayout = () => {
  ...
  const [collapsedGroups, setCollapsedGroups] = useState<Set<string>>(new Set(['Hiệu năng', 'Màn hình', 'Kết nối', 'Pin & Sạc', 'Thiết kế']));
  ...
  const clearTempFolder = api.cloudinary.clearTempFolder.useMutation();
  ...
  const handleUpload = async () => {
    ...
    // Clear temp folder on first upload for new product
    if (!isEdit && !tempCleared) {
      try {
        await clearTempFolder.mutateAsync();
        setTempCleared(true);
      } catch (error) {
    ...
    window.cloudinary.openUploadWidget(
      {
        ...
        multiple: true,
        folder: isEdit ? `kara-shop/products/${id}` : 'kara-shop/products/temp',
      },
      (error: unknown, result: { event: string; info: { secure_url: string } }) => {
        if (!error && result && result.event === 'success') {
          ...
          // Sync with DB immediately if in edit mode (not normally used for "new" but logic should be robust)
          if (isEdit && id) {
  ...
  const updateInclusion = (index: number, value: string) => {
    ...
    const inclusion = newInclusions[index];
    if (inclusion) {
      inclusion.itemName = value;
    }
      ...
      <div className="container mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="relative mb-10 group">
          ...
          <button
            ...
            className="absolute -left-4 top-1/2 z-10 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-md border border-gray-100 text-gray-600 hover:text-violet-600 opacity-0 group-hover:opacity-100 transition-all font-bold"
          >
          ...
          <button
            ...
            className="absolute -right-4 top-1/2 z-10 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-md border border-gray-100 text-gray-600 hover:text-violet-600 opacity-0 group-hover:opacity-100 transition-all font-bold"
          >
          ...
          {currentStep === 4 && (
            <div className="animate-fadeIn space-y-6">
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-[2fr_2fr_3fr_auto] items-end">
                    <div>
                      ...
                      >
                        ...
                        <option value="ADD_NEW_GROUP" className="font-bold text-violet-600">
                          + Thêm nhóm tính năng mới
                        </option>
                      </select>
                    </div>
                    ...
                    <button
                      ...
                      className="flex h-[46px] items-center gap-2 rounded-xl bg-violet-600 px-6 py-3 text-sm font-bold text-white transition-all hover:bg-violet-700 hover:shadow-md"
                    >
                      <FiPlus /> Thêm
                    </button>
                  </div>
              ...
              <div className="space-y-4">
                {attributeGroups.map((groupName) => {
                  ...
                  return (
                    <div
                      key={groupName}
                      id={`group-${groupName}`}
                      className="scroll-mt-4 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm"
                    >
                      {/* Group Header */}
                      <div className="flex items-center justify-between border-b border-gray-100 bg-gradient-to-r from-gray-50 to-gray-100 p-4 transition-all hover:from-gray-100 hover:to-gray-150">
                        <button
                          type="button"
                          onClick={() => toggleGroupCollapse(groupName)}
                          className="flex flex-1 items-center gap-3"
                        >
                          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-100 text-sm font-bold text-violet-600">
                            {groupAttrs.length}
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <select
                            ...
                            className="rounded-lg border border-gray-200 bg-white p-1.5 text-xs focus:border-violet-500 outline-none"
                            value={groupName}
                          >
                        ...
                        <div className="space-y-3 bg-white p-4">
                          {groupAttrs.map((attr) => {
                            ...
                            return (
                              <div
                                key={index}
                                className="grid items-end gap-3 rounded-xl border border-gray-100 bg-gray-50 p-3 md:grid-cols-[1.5fr_2.5fr_1fr_auto]"
                              >
                                <div>
                                  <label className="mb-1.5 block text-[10px] font-bold uppercase text-gray-400">
                                    Tên thông số
                                  </label>
                                  <input
                                    type="text"
                                ...
                                <button
                                  ...
                                  className="flex h-10 w-10 items-center justify-center rounded-lg border border-gray-200 bg-white text-red-400 transition-colors hover:border-red-200 hover:bg-red-50 hover:text-red-500"
                                >
                                  <FiTrash2 size={16} />
                                </button>
                              </div>
                ...
                {(() => {
                  const ungrouped = formData.attributes.filter(
                    (attr) => !attr.groupName || attr.groupName.trim() === ''
                  );
                  if (ungrouped.length === 0) return null;

                  return (
                    <div className="overflow-hidden rounded-2xl border border-dashed border-gray-300 bg-gray-50">
                      <div className="flex items-center justify-between border-b border-yellow-100 bg-yellow-50 p-4">
                        ...
                        <div className="flex items-center gap-2">
                          <label className="text-[10px] font-bold uppercase text-yellow-700">
                            Chuyển toàn bộ sang:
                          </label>
                          <select
                            onChange={(e) => moveAllAttributesToGroup(undefined, e.target.value)}
                            className="rounded-lg border border-yellow-200 bg-white p-1.5 text-xs focus:border-yellow-500 outline-none"
                            value=""
                          >
                            <option value="">-- Chọn nhóm --</option>
                            {attributeGroups.map((group) => (
                      ...
                      <div className="space-y-3 p-4">
                        {ungrouped.map((attr) => {
                          ...
                          return (
                            <div
                              key={index}
                              className="grid items-end gap-3 rounded-xl border border-gray-200 bg-white p-3 md:grid-cols-[1.5fr_2.5fr_1fr_auto]"
                            >
                              <div>
                                <label className="mb-1.5 block text-[10px] font-bold uppercase text-gray-400">
                                  Tên thông số
                              ...
                              <button
                                type="button"
                                onClick={() => removeAttribute(index)}
                                className="flex h-10 w-10 items-center justify-center rounded-lg bg-gray-50 text-red-400 transition-colors hover:bg-red-50 hover:text-red-500"
                              >
                                <FiTrash2 size={16} />
                              </button>
                            </div>
                ...
                {formData.attributes.length === 0 && (
                  <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 py-16 text-center">
                    <p className="mb-2 text-sm text-gray-400">Chưa có thông số kỹ thuật.</p>
                    <p className="text-xs text-gray-400">Chọn nhóm và bấm &quot;Thêm&quot; để bắt đầu.</p>
                  </div>
                )}
              </div>
            </div>
                ...
                {currentStep < STEPS.length - 1 ? (
                  <>
                    {currentStep >= 1 && (
                      ...
                      >
                        {createProduct.isLoading ? 'Đang lưu...' : 'Lưu ngay'}
                      </button>
                    )}
                    <button
                      key="btn-next"
